...
# -*- coding=utf-8 -*-
"""
模板样式 https://gjbtrain.linkedcare.cn/
"""
import copy
import importlib
import random
...
class Page(default.Page, Style):
    ...
    def create(self, o_content=None):
        ...
        if self.top_id and self.top_id.find("id:") < 0:
            o_grid.set_col_attr(col=0, width=self.WIDTH_LEFT, scroll={"overflow_y": "auto"})
            o_grid.add_widget(col=0, widget=self.create_left())
            o_grid.set_col_attr(col=1, width="100%%-%s" % self.WIDTH_LEFT)

        elif self.menu_id is not None:
            url = common.make_url(const.URL_LEFT, param={const.UPN_MENU_ID: self.menu_id})

            o_grid.set_col_attr(col=0, width=self.WIDTH_LEFT, url=url, scroll={"overflow_y": "auto"})
            o_grid.set_col_attr(col=1, width="100%%-%s" % self.WIDTH_LEFT)
    ...
    def create_top(self):
        ...
        focus_color = common.calc_focus_color(self.o_theme.top_bg_color)
        ...
        for o_widget in settings.V_TOP_ADD_ITEMS or []:
            if getattr(o_widget, "width", None):
        ...
        for o_widget in lst_widget:
            ...
            o_widget.font_color = font_color
            o_widget.focus_bg_color = focus_color
    ...
    def create_left(self):
        o_panel = widgets.Panel(width="100%", height="100vh",
                                horizontal="center", vertical="top", scroll={"overflow": "auto"},
        ...
        active = self.o_theme.menu.get("active", {})
        if self.o_theme.theme_color == self.o_theme.left_bg_color:
            active["bg_color"] = common.calc_focus_color(self.o_theme.left_bg_color)
        else:
            active["bg_color"] = self.o_theme.theme_color

        active["font_color"] = "#FFFFFF"
        active["border_radius"] = self.o_theme.theme_round

        focus = self.o_theme.menu.get("focus", {})
        focus["bg_color"] = common.calc_focus_color(self.o_theme.left_bg_color)
...
class ChangeForm(default.ChangeForm, Style):
                        ...
                        for (i, field_name) in enumerate(lst_sub_field):
                            ...
                            lst_widget1, lst_widget2 = \
                                field_translate.make_field_widget(self.request, obj,
                                                                  ...
                                                                  readonly_text=self.is_readonly_text,
    ...
    def make_opera_area(self, obj=None):
        ...
        if self.display_mode == const.DM_FORM_POPUP:
            ...
            o_step = step.Post(url=url, submit_type="layer")
            ...
            o_panel.set_attr_value("hover", {"left": self.MARGIN_LEFT, "bottom": 0, "mode": "window"})
            # 自定义
            ...
            if (obj is not None and self.model_admin.has_change_permission(self.request, obj)) or \
