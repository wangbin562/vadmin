...
class StyleManageApi(object):
    ...
    def get_style(key, lst_style, dict_label_style, default_style=None, not_key=None):
        """
        先获取当前样式（标签中的style关键字），再获取css中标签样式，再获取上级样式
        Args:
            key:
            lst_style:
            dict_label_style:
...
class DocApi(object):
    ...
    def set_paragraph_style(paragraph, lst_style, dict_label_style):
        ...
        if style is not None:
            ...
            line_spacing = StyleManageApi.line_spacing_change(style, lst_style,
                                                              dict_label_style)  # 行距(没有单位）
            DocApi.set_line_spacing(paragraph, line_spacing)

        else:
            paragraph.paragraph_format.line_spacing = 1.0

        paragraph.alignment = WD_TAB_ALIGNMENT.CENTER
        # 设置文字对齐方式
