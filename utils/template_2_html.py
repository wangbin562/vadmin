...
class Cell(object):
    def __init__(self):
        ...
        self.font_color = None  # 文字颜色
        self.font_size = None  # 文字大小
        self.cell_style = []  # 单元格样式（斜体，下划线等）
        self.rowspan = None  # 行合并
        self.colspan = None  # 列合并
        self.wrap_text = False  # 自动换行
...
class BasePage(object):
    ...
    def move_down(self, copy_style=True, copy_row_height=True, add_row=True):
        ...
        if add_row:
            ...
            self.dict_w_cell.setdefault(self.w_row, {})[self.w_col] = Cell()
            if copy_row_height:
            ...
            self.w_row += 1
            self.dict_w_cell.setdefault(self.w_row, {})[self.w_col] = Cell()
        # self.w_cell = self.w_sheet.cell(row=w_row, column=w_col)
            ...
            if copy_col_width:
                for i in range(col_span + 1):
                    ...
                    if col not in self.dict_width:
                        self.dict_width[col] = self.lst_col_width[r_col + i - 1]
    ...
    def write_style(self, **kwargs):
        ...
        写入样式
        """
        o_cell = self.dict_w_cell[self.w_row][self.w_col]

        if "background_color" in kwargs:
            o_cell.background_color = kwargs["background_color"][1:]

        if "font_color" in kwargs:
            o_cell.cell_style.append("color:#%s" % kwargs["font_color"][1:])

        if "font_size" in kwargs:
            o_cell.cell_style.append("font-size:%spx" % kwargs["font_size"])
    ...
    def write(self, text):
        o_cell = self.dict_w_cell[self.w_row][self.w_col]
        if o_cell.value in [None, ""]:
            o_cell.value = text
        else:
            o_cell.value = o_cell.value + text
    ...
    def init_cell(self, copy_style=True, dict_style=None):
        ...
        if b_merged:
            if row_span:
                self.dict_w_cell[self.w_row][self.w_col].rowspan = row_span + 1

            if col_span:
                self.dict_w_cell[self.w_row][self.w_col].colspan = col_span + 1

        col_num = len(self.lst_col_width)
        for i in range(col_span + 1):
            col = self.w_col + i
            if (col not in self.dict_width) and (r_col + i - 1 < col_num):
                self.dict_width[col] = self.lst_col_width[r_col + i - 1]

        row_num = len(self.lst_row_height)
        for i in range(row_span + 1):
            row = self.w_row + i
            if (row not in self.dict_height) and (r_row + i - 1 < row_num):
                self.dict_height[row] = self.lst_row_height[r_row + i - 1]
        ...
        if copy_style:
            ...
            # font-size:40px
            if o_font.size and o_font.size != 11:
                o_cell.cell_style.append("font-size:%spx" % pt_2_px(o_font.size))

            #  text-decoration: underline
            if o_font.underline:
                o_cell.cell_style.append("text-decoration:underline")

            if o_font.strike:
                o_cell.cell_style.append("text-decoration:line-through")

            if o_font.italic:
                o_cell.cell_style.append("font-style:italic")

            # 文字颜色
            color = get_color(o_font.color)
            if color and color not in ["000000"]:
                o_cell.cell_style.append("color:#%s" % color)
            ...
            if o_alignment.horizontal:
                if o_alignment.horizontal == "left":
                    o_cell.cell_style.append("text-align:left")
                elif o_alignment.horizontal == "right":
                    o_cell.cell_style.append("text-align:right")
                elif o_alignment.horizontal == "center":
                    o_cell.cell_style.append("text-align:center")

            if o_alignment.vertical:
                # if o_alignment.vertical == "center":
                #     o_cell.cell_style.append("vertical-align:center")
                if o_alignment.vertical == "top":
                    o_cell.cell_style.append("vertical-align:top")
            else:
                o_cell.cell_style.append("vertical-align:bottom")
            # 边框
            # self.w_cell.border = copy.copy(self.lst_border[border_idx])

            # 背景颜色
            color = get_color(self.lst_fill[fill_idx].fgColor)
            if color and color not in ["FFFFFF"]:
                o_cell.background_color = color

    def create(self, **kwargs):
        self.init_para(**kwargs)
        for row, lst_row in enumerate(self.lst_cell):
            # w_row = row + self.dict_add_row.get(0, 0)
            for col, r_cell in enumerate(lst_row):
                ...
                # 前面循环已处理
                if (self.w_row in self.dict_w_cell) and (self.w_col in self.dict_w_cell[self.w_row]):
                    continue
        ...
        _w = self.output.write
        _w('<div><style>table td{padding-left:5px;}</style>')
        _w('<table border="1" cellspacing="0" cellpadding="0" '
           'style="border-collapse:collapse;border:1px solid #000000;table-layout:fixed;overflow:hidden;text-overflow:ellipsis;'
           'color:#000000;font-size:14.67px;word-break: break-all;margin: 0 auto;">\n')
        _w('<colgroup>\n')
        lst_key = list(self.dict_width.keys())
        lst_key.sort()
        for key in lst_key:
            col_width = self.dict_width[key]
            _w('<col width="%spx"></col>' % col_width)
            _w('\n')
        _w('</colgroup>')
        _w('\n')
        ...
        for row in range(1, max_row + 1):
            height = self.dict_height[row]
            _w('<tr style="height:%spx;">' % height)
            _w('\n')
            for col in range(1, max_col + 1):
                if row in self.dict_w_cell and col in self.dict_w_cell[row]:
                    ...
                    o_cell = self.dict_w_cell[row][col]
                    if o_cell.rowspan is not None:
                        _w(' rowspan="%s"' % o_cell.rowspan)

                    if o_cell.colspan is not None:
                        _w(' colspan="%s"' % o_cell.colspan)

                    if o_cell.cell_style:
                        _w(' style="%s"' % ";".join(o_cell.cell_style))

                    if o_cell.background_color is not None:
                        _w(' bgcolor="#%s"' % o_cell.background_color)

                    _w('>')

                    if o_cell.value not in ["", None]:
                        if o_cell.rowspan is None:
                            pass
                        else:
                            height = 0.0
                            for i in range(o_cell.rowspan):
                                height += self.dict_height[row + i]

                        if o_cell.wrap_text:
                            # _w('<div style="max-height:%s;">' % height)
                            _w(str(o_cell.value))
                            # _w('</div>')
                        else:
                            _w('<div style="max-height:%spx;overflow:hidden;">' % height)
                            _w(str(o_cell.value))
                            _w('</div>')
...
class Compiler(object):
    ...
    def parse_excel(self):
        ...
        for row in range(1, self.max_row + 1):
            ...
            for col in range(1, self.max_col + 1):
                ...
                if lst_node:
                    # node = lst_node[0]
                    text = "def write_%s_%s(self, _w):" % (row, col)
    ...
    def generate_code(self):
        ...
        # lst_code.append('from %s.%s import BasePage' % (os.path.split(dir)[1], os.path.splitext(path_name)[0]))
        lst_code.append('from utils.template_2_html import BasePage')
        lst_code.append('')
        ...
        lst_code.append('''_move_down = None
_write_style = None
...
def generate(template_file, is_compile=False, max_row=100, max_col=30, **kwargs):
    ...
    module_name = "excel_2_html_%s" % os.path.splitext(name)[0]
    p = Pinyin()
...
if __name__ == "__main__":
    ...
    begin = time.time()
    # output = generate(r"C:\wangbin\code_sync\code_sync\hmi\server\help\html\0-介绍.xlsx", mandatory_compile=True, border_width=0)
    # lst_time = ["2018-10-10"] * 10
    # dict_para = {}
    # dict_para["lst_1"] = [1] * 10
    # dict_para["lst_2"] = [2] * 10
    # dict_para["lst_3"] = [3] * 10
    # dict_para["lst_4"] = [4] * 10
    # dict_para["lst_5"] = [5] * 10
    # dict_para["lst_6"] = [6] * 10
    # dict_para["lst_7"] = [7] * 10
    # dict_para["lst_8"] = [8] * 10
    # dict_para["lst_9"] = [9] * 10
    # dict_para["lst_10"] = [10] * 10
    # dict_para["lst_11"] = [11] * 10
    #
    # lst_1 = [1] * 10
    # lst_2 = [2] * 10
    # lst_3 = [3] * 10
    ...
    # lst_5_b = [14] * 10
    # lst_5_c = [15] * 10
    # lst_surgical_1_a = [16] * 10
    # lst_surgical_1_b = [17] * 10
    # lst_surgical_1_c = [18] * 10
    # lst_surgical_2_a = [19] * 10
    # lst_surgical_2_b = [20] * 10
    # lst_surgical_2_c = [21] * 10
    # output = generate(r"中央空调报表1.xlsx", is_compile=True, lst_time=lst_time,
    #                        lst_2=dict_para["lst_2"], lst_3=dict_para["lst_3"], lst_4=dict_para["lst_4"], lst_5=dict_para["lst_5"],
    #                        lst_6=dict_para["lst_6"], lst_7=dict_para["lst_7"], lst_8=dict_para["lst_8"], lst_9=dict_para["lst_9"],
    #                        lst_10=dict_para["lst_10"], lst_11=dict_para["lst_11"], dict_para=dict_para)

    output = generate(r"C:\wangbin\code_sync\code_sync\zhwhhuiyu\zhonghe\templates\5\4.xlsx", is_compile=True,
                      lst_progress=["2018-01-01", "2018-01-02", "2018-01-03"],
                      week_date="2018-01-02")

    fh = open("1.html", "w", encoding='utf-8')
    fh.write("%s" % output.getvalue())
    fh.close()
    end = time.time()
    print(end - begin)
